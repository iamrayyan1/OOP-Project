...
class easyClass : public enemyClass, public gamePlay {
  ...
  public:
    ...
    void drawEnemy override(int ind) {
        if (enemyFlag[ind] == true) {
            gotoxy(enemyX[ind], enemyY[ind]);
            cout << "|  |";
            gotoxy(enemyX[ind], enemyY[ind]);
...
void hardClass::play() {
    ...
    carPos = -1 + WIN_WIDTH / 2;
    score = 0;
    for (int i = 0; i < 4; ++i) // Change loop condition to 4 to cover all enemies
        enemyFlag[i] = true; // Set all flags to true initially
    for (int i = 0; i < 4; ++i) // Correct the loop condition and initialization
        enemyY[i] = 1;
    ...
    for (int i = 0; i < 4; ++i) { // Change loop condition to 4 to cover all enemies
        genEnemy(i);
        drawEnemy(i); // Draw each enemy car
    }
